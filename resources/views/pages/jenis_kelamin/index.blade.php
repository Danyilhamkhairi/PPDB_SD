...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between page-header">
        <div>
            <h1 class="h3 mb-0 page-title">Manajemen Jenis Kelamin</h1>
            <p class="page-subtitle">Kelola data jenis kelamin untuk pendaftaran peserta</p>
        </div>
        <a href="{{route('jenis_kelamin.create')}}" class="btn btn-primary shadow">
    ...
    <div class="card shadow mb-4">
        <div class="card-header d-flex flex-row align-items-center justify-content-between">
            <h6 class="m-0 font-weight-bold text-primary">Daftar Jenis Kelamin</h6>
                            ...
                            <td>
                                @if (strtolower($item->jenis_kelamin) == 'laki-laki')
                                    <span class="badge-gender badge-male">
                                        <i class="fas fa-mars mr-1"></i> {{$item->jenis_kelamin}}
                                    </span>
                                @elseif (strtolower($item->jenis_kelamin) == 'perempuan')
                                    <span class="badge-gender badge-female">
                                        <i class="fas fa-venus mr-1"></i> {{$item->jenis_kelamin}}
                                    </span>
                                @else
                                    <span class="font-weight-medium">{{$item->jenis_kelamin}}</span>
                                @endif
                            </td>
                            <td>
                                <a href="{{route('jenis_kelamin.edit', $item->id)}}" class="btn btn-success btn-action mr-2 shadow-sm">
                                    <i class="fas fa-edit action-icon"></i> Ubah
                                </a>
                                <form method="post" class="d-inline-block" action="{{route('jenis_kelamin.destroy', $item->id)}}">
                                    @method('DELETE')
                                    @csrf
                                    <button type="submit" class="btn btn-danger btn-action shadow-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
