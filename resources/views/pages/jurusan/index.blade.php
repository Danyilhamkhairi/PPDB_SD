...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between page-header">
        <div>
            <h1 class="h3 mb-0 page-title">Manajemen Jurusan</h1>
            <p class="page-subtitle">Kelola daftar jurusan untuk pendaftaran peserta</p>
        </div>
        <a href="{{route('jurusan.create')}}" class="btn btn-primary shadow">
            <i class="fas fa-plus fa-sm mr-2"></i>Tambah Jurusan
        </a>
    </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge-major">
                                    <i class="fas fa-graduation-cap mr-1"></i> {{$item->jurusan}}
                                </span>
                            </td>
                            <td>
                                <a href="{{route('jurusan.edit', $item->id)}}" class="btn btn-success btn-action mr-2 shadow-sm">
                                    <i class="fas fa-edit action-icon"></i> Ubah
                                </a>
                                <form method="post" class="d-inline-block" action="{{route('jurusan.destroy', $item->id)}}">
                                    @method('DELETE')
                                    @csrf
                                    <button type="submit" class="btn btn-danger btn-action shadow-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus jurusan ini?')">
